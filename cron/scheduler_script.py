...
def _run_job_script(
    ...
    instead of the scripts-dir parent. See #69396. script_args: Optional string of command-line
    arguments to pass to the script (e.g. ``"--save-snapshot"``); shell-quoted via shlex.
    """
    ...
    if argv is None:
        return False, err
    # Append script_args to the interpreter argv (custom: script-args support).
    if script_args:
...
def _run_job_script_with_claim_heartbeat(
    ...
    if not (isinstance(schedule, dict) and schedule.get("kind") == "once" and owner):
        return _run_job_script(script_path, workdir=workdir, cancel_event=cancel_event, script_args=script_args)
