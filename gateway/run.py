...
if _config_path.exists():
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            ...
            if (
                "gateway_timeout" in _agent_cfg
                and "HERMES_AGENT_TIMEOUT" not in os.environ
            ):
...
class GatewayRunner:
    ...
    async def _session_expiry_watcher(self, interval: int = 300):
        ...
        while self._running:
            try:
                ...
                if _expired_entries:
                    _flushed = sum(1 for _, e in _expired_entries if e.memory_flushed)
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if _quick_key in self._running_agents and _stale_ts:
            ...
            _wall_ttl = (
                max(_raw_stale_timeout * 10, 7200)
                if _raw_stale_timeout > 0
                else float("inf")
            )
            _should_evict = (
                                ...
                                if _cp_url and _cp_url == _hyg_base_url.rstrip("/"):
                                    ...
                                    if isinstance(_cp_models, dict):
                                        ...
                                        if isinstance(_cp_model_cfg, dict):
                                            _cp_ctx = _cp_model_cfg.get(
                                                "context_length"
                                            )
                                            if _cp_ctx is not None:
                                                _hyg_config_context_length = int(
                                                    _cp_ctx
                                                )
                                    break
                    except (TypeError, ValueError):
    ...
    async def _handle_commands_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            if skill_cmds:
                ...
                for cmd in sorted(skill_cmds):
                    desc = (
                        skill_cmds[cmd].get("description", "").strip()
                        or "Skill command"
                    )
        ...
        if page != requested_page:
            lines.append(
                f"_(Requested page {requested_page} was out of range, showing page {page}.)_"
            )
        return "\n".join(lines)
            ...
            try:
                ...
                for p in providers:
                    ...
                    if p["models"]:
                        ...
                        extra = (
                            f" (+{p['total_models'] - len(p['models'])} more)"
    ...
    async def _handle_models_command(self, event: MessageEvent) -> str:
        """Handle /models command — list available models with selection menu."""
        import json
        from pathlib import Path

        raw_args = event.get_command_args().strip().lower()
        show_free_only = "--free" in raw_args
        provider_filter = None

        if "zen" in raw_args or "opencode" in raw_args:
            provider_filter = "opencode-zen"
        elif "openrouter" in raw_args or "router" in raw_args:
            provider_filter = "openrouter"

        rankings_file = _hermes_home / "state" / "unified_model_rankings.json"
        if not rankings_file.exists():
            return "No model rankings found. Run `~/.hermes/scripts/unified_model_ranking.py` first."

        try:
            with open(rankings_file) as f:
                data = json.load(f)
        except Exception as e:
            return f"Error reading rankings: {e}"

        results = data.get("results", [])
        timestamp = data.get("timestamp", "unknown")

        if provider_filter:
            results = [r for r in results if provider_filter in r.get("provider", "")]
        if show_free_only:
            results = [
                r
                for r in results
                if "free" in r.get("model_id", "").lower()
                or r.get("source") == "opencode-zen"
            ]
        ...
        if is_telegram:
            ...
            if telegram_adapter and hasattr(telegram_adapter, "send_models_menu"):
                models_with_tps = []
                for prov, models in sorted(by_provider.items()):
                    for m in models:
                        models_with_tps.append(
                            {
                                "model_id": m.get("model_id", m.get("slug", "unknown")),
                                "tps": m.get("tps", 0),
                ...
                if provider_filter:
                    title = f"Model Rankings ({provider_filter})"
                elif show_free_only:
                    title = "Free Model Rankings"

                try:
                    await telegram_adapter.send_models_menu(
                        chat_id=str(source.chat_id),
                        models=models_with_tps,
                        title=title,
                    )
                    return ""
                except Exception as e:
                    logger.warning("Failed to send models menu: %s", e)

        lines = [
            f"**Model Rankings** — {timestamp.split('T')[0] if 'T' in timestamp else timestamp}",
            "",
        ]
        ...
        for prov, models in sorted(by_provider.items()):
            ...
            for m in models[:15]:
                ...
                if tps >= 1000000:
                    tps_str = f"{tps / 1000000:.1f}M"
                elif tps >= 1000:
                    tps_str = f"{tps / 1000:.1f}K"
                else:
                    tps_str = f"{tps:.0f}"

                status = "✓" if m.get("status") == "ok" else "✗"
                lines.append(f"{model_index:2}. `{model_id}` — {tps_str} t/s {status}")

        lines.append("")
        lines.append("`/model <name>` — switch to a model")
        if provider_filter is None:
            lines.append("`/models openrouter` — OpenRouter models only")
            lines.append("`/models zen` — OpenCode Zen models only")
        lines.append("`/models --free` — free models only")
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        if is_telegram:
            telegram_adapter = self.adapters.get(Platform.TELEGRAM)
            if telegram_adapter and hasattr(telegram_adapter, "send_provider_menu"):
                rankings_file = _hermes_home / "state" / "unified_model_rankings.json"
                provider_counts = {}
                ...
                providers_with_counts = [
                    {
                        ...
                        "label": "OpenCode Zen",
                        "count": provider_counts.get("opencode-zen", 0),
                    },
                    {
                        "provider": "openrouter",
                        "label": "OpenRouter",
                        "count": provider_counts.get("openrouter", 0),
                    },
                ]

                try:
                    await telegram_adapter.send_provider_menu(
                        chat_id=str(source.chat_id),
                        providers=providers_with_counts,
                    )
                    return ""
                except Exception as e:
                    logger.warning("Failed to send provider menu: %s", e)
    ...
    async def _run_btw_task(
        ...
        if not adapter:
            logger.warning(
                "No adapter for platform %s in /btw task %s", source.platform, task_id
            )
            return
        ...
        try:
            ...
            if running_agent and running_agent is not _AGENT_PENDING_SENTINEL:
                history_snapshot = list(
                    getattr(running_agent, "_session_messages", []) or []
                )
            else:
                session_entry = self.session_store.get_or_create_session(source)
                history_snapshot = self.session_store.load_transcript(
                    session_entry.session_id
                )
            ...
            for media_path in media_files or []:
                try:
                    await adapter.send_file(
                        chat_id=source.chat_id, file_path=media_path
                    )
                except Exception:
                    pass
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        logger.info(
            "User approved %d dangerous command(s) via /approve%s", count, scope_msg
        )
    ...
    async def _watch_update_progress(
        ...
        if not adapter or not chat_id:
            logger.warning(
                "Update watcher: cannot resolve adapter/chat_id, falling back to completion-only"
            )
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    logger.info(
                        "Update finished (exit=%s), notified %s", exit_code, session_key
                    )
                except Exception as e:
            ...
            if prompt_path.exists() and session_key:
                try:
                    ...
                    if prompt_text:
                        ...
                        sent_buttons = False
                        if (
                            getattr(type(adapter), "send_update_prompt", None)
                            is not None
                        ):
                            try:
                                ...
                                logger.debug(
                                    "Button-based update prompt failed: %s", btn_err
                                )
                        if not sent_buttons:
        ...
        if not exit_code_path.exists():
            ...
            try:
                await adapter.send(
                    chat_id, "❌ Hermes update timed out after 30 minutes."
                )
            except Exception:
                pass
            for p in (
    ...
    async def _run_agent(
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            _executor_task = asyncio.ensure_future(loop.run_in_executor(None, run_sync))
            ...
            if _inactivity_timeout:
                ...
                _activity = {}
                if _timed_out_agent and hasattr(
                    _timed_out_agent, "get_activity_summary"
                ):
                    try:
...
async def start_gateway(
    ...
    if verbosity is not None:
        _stderr_level = {0: logging.WARNING, 1: logging.INFO}.get(
