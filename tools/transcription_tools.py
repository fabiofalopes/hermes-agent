...
# ---------------------------------------------------------------------------
# Voice note chunking integration
# ---------------------------------------------------------------------------

_VOICE_NOTE_PATH = Path("/home/fabio/voice_note_repo")
_VOICE_NOTE_SRC = _VOICE_NOTE_PATH / "src"
if _VOICE_NOTE_SRC.exists():
    for _p in (str(_VOICE_NOTE_PATH), str(_VOICE_NOTE_SRC)):
        if _p not in sys.path:
            sys.path.insert(0, _p)

_HAS_VOICE_NOTE_CHUNKING = False
try:
    from src.api.groq_client import GroqWhisperClient

    _HAS_VOICE_NOTE_CHUNKING = True
except ImportError:
    GroqWhisperClient = None  # type: ignore[misc]
...
def _has_openai_audio_backend() -> bool:
    ...
    return bool(
        resolve_openai_audio_api_key() or resolve_managed_tool_gateway("openai-audio")
    )
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    """Transcribe using Groq Whisper API with voice_note smart chunking."""
    ...
    if _HAS_VOICE_NOTE_CHUNKING:
        return _transcribe_groq_chunked(file_path, model_name)
    return _transcribe_groq_direct(file_path, model_name)


def _transcribe_groq_chunked(file_path: str, model_name: str) -> Dict[str, Any]:
    """Transcribe using voice_note's GroqWhisperClient with ffmpeg chunking."""
    try:
        client = GroqWhisperClient()
        result = client.transcribe(file_path, model=model_name)

        if result is None:
            return {
                "success": False,
                "transcript": "",
                "error": "Chunked transcription returned None",
            }

        logger.info(
            "Transcribed %s via Groq chunked (%s, %d chars)",
            Path(file_path).name,
            model_name,
            len(result),
        )
        return {"success": True, "transcript": result, "provider": "groq_chunked"}

    except Exception as e:
        logger.error("Groq chunked transcription failed: %s", e, exc_info=True)
        return {
            "success": False,
            "transcript": "",
            "error": f"Chunked transcription failed: {e}",
        }


def _transcribe_groq_direct(file_path: str, model_name: str) -> Dict[str, Any]:
    """Transcribe using Groq Whisper API directly (no chunking, 25MB limit)."""
    if not _HAS_OPENAI:
        return {
            "success": False,
            "transcript": "",
